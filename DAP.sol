 ...
 *Git: https://github.com/dappad/DAP.git
*/
...
contract Token is Context, IERC20, Ownable {
    ...
    uint8 public _walletteamFee = 0;
    uint8 private _previousWalletteamFee = _walletteamFee;
    ...
    address payable public feeWalletteam;

    bool walletFeeInBNB = false;
    bool walletteamFeeInBNB = false;
    ...
    constructor (string memory tokenName,
        ...
        feeWalletteam = wallet.walletteam;
        walletFeeInBNB = wallet.walletFeeInBNB;
        walletteamFeeInBNB = wallet.walletteamFeeInBNB;
        ...
        require(fee.setWalletteamFee >= 0 && fee.setWalletteamFee <=maxWalletFee,"WFT err");
        ...
        _walletteamFee = fee.setWalletteamFee;
    ...
    function setAllFeePercent(uint8 taxFee, uint8 liquidityFee, uint8 burnFee, uint8 walletFee, uint8 buybackFee, uint8 walletteamFee, uint8 rewardFee) external onlyOwner() {
        ...
        require(walletteamFee >= 0 && walletteamFee <=maxWalletFee,"WFT err");
    ...
    function setFeeWalletteam(address payable newFeeWallet) external onlyOwner {
        require(newFeeWallet != address(0), "ZERO ADDRESS");
        feeWalletteam = newFeeWallet;
    }
    ...
    function setWalletteamFeeTokenType(bool inBNB) external onlyOwner {
        walletteamFeeInBNB = inBNB;
    }
    ...
    function calculateLiquidityFee(uint256 _amount) private view returns (uint256) {
        return _amount.mul(_liquidityFee + _burnFee + _walletFee + _buybackFee + _walletteamFee + _rewardFee).div(
            10**2
        );
    }
    
    function removeAllFee() private {
        if(_taxFee == 0 && _liquidityFee == 0 && _burnFee == 0 && _walletFee == 0 && _buybackFee == 0 && _walletteamFee == 0 && _rewardFee == 0) return;
        ...
        _previousWalletteamFee = _walletteamFee;
    ...
    function restoreAllFee() private {
        ...
        _walletteamFee = _previousWalletteamFee;
    ...
    function swapAndLiquify(uint256 contractTokenBalance) private lockTheSwap {
        ...
        //burn
        uint8 totFee  = _burnFee + _walletFee + _liquidityFee + _buybackFee + _walletteamFee + _rewardFee; 
        ...
        if(_walletteamFee != 0){
            spentAmount = contractTokenBalance.div(totFee).mul(_walletteamFee);
            
            if(!walletteamFeeInBNB){
                uint256 currentBalance = balanceOf(feeWalletteam);
                _tokenTransferNoFee(address(this), feeWalletteam, spentAmount);
                setBalance(payable(feeWalletteam), balanceOf(feeWalletteam), currentBalance);
            }
            else{
                uint256 initialBalance = address(this).balance;
                // swap tokens for ETH
                ...
                // how much ETH did we just swap into?
                uint256 newBalance = address(this).balance.sub(initialBalance);
                transferEth(feeWalletteam, newBalance);
            }
            totSpentAmount = totSpentAmount + spentAmount;
        }
